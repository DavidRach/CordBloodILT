...
**Supplementary Figure 14: Sex-based differences in ILT marker expression after polyclonal stimulation.** (A) The beeswarm plots show the proportions of cells expressing individual markers listed on the x-axis for V𝛿2 (A), MAIT (B) and NKT (C) cells, respectively. The symbols show individual values, boxplots show median and IQR, with whiskers indicating the +/- 1.5 IQR range. Shape and color identify infant sex.

# Code

```{r}
StorageLocation <- file.path("SupplementalFigures", "SupplementalFigure14")
...
Plot <- Utility_MarkerPlots(data=data, panel=panel, myfactor="infant_sex", shape_palette = shape_sex,
                            ...
                            filename="Vd2_PMAAntibody_sex", corral.width=0.8)
...
Plot <- Utility_MarkerPlots(data=data, panel=panel, myfactor="infant_sex", shape_palette = shape_sex,
                            fill_palette = fill_sex, XAxisLevels = XAxisLevels, savePlot=FALSE,
                            filterForThese = XAxisLevels, outpath=StorageLocation,
                            filename="MAIT_PMAAntibody_sex", corral.width=0.8)
...
Plot <- Utility_MarkerPlots(data=data, panel=panel, myfactor="infant_sex", shape_palette = shape_sex,
                            fill_palette = fill_sex, XAxisLevels = XAxisLevels, savePlot=FALSE,
                            filterForThese = XAxisLevels, outpath=StorageLocation,
                            filename="NKT_PMAAntibody_sex", corral.width=0.8)
